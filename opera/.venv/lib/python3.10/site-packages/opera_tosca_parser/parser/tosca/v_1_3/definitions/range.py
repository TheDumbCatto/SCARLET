...
# NOTE: We do some pretty horrible checks in the validate function. There are two reasons for this mess:
#
#  1. TOSCA's range type is a bit ugly to parse because upper bound can be string (UNBOUNDED) and should be treated as
#     infinity.
#  2. Python's bool type subclasses int, which makes isinstance(False, int) trusty. This is why we have separate checks
#     for bool.
...
class Range(Base):
    ...
    @classmethod
    def build(cls, yaml_node: Node) -> Range:
        """
        Build Range object from YAML node
        :param yaml_node: YAML node
        :return: Range object
        """
        lo, hi = yaml_node.value

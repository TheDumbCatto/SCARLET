...
from opera_tosca_parser.parser.tosca.v_2_0.constants import OperationHost, StandardInterfaceOperation, \
...
class CollectorMixin(Entity):
    ...
    def collect_properties(self, service_ast: Dict[str, Any]) -> Dict[str, Value]:
        ...
        for key, prop_definition in definitions.items():
            ...
            if not prop_required and prop_has_default:
                self.abort(
                    f"The default keyname shall not be defined for property: {key} because it is not required and will "
                    f"stay undefined.", self.loc
                )
